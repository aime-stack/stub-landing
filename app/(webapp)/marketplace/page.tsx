...
function AddProductModal({ onClose, onRefresh }: { onClose: () => void, onRefresh: () => void }) {
  ...
  return (
    <div onClick={e => { if (e.target === e.currentTarget) onClose(); }}
      ...
      <div style={{ background: 'white', borderRadius: 24, width: '100%', maxWidth: 550, maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 24px 60px rgba(0,0,0,0.18)' }}>
        ...
        <div style={{ padding: 20, display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          {[
            ...
            <div key={f.label}>
              ...
              <input value={f.val} onChange={e => f.set(e.target.value)} placeholder={f.ph} type={f.type}
                style={{ width: '100%', height: 44, padding: '0 14px', borderRadius: 12, border: '1.5px solid #E5E7EB', fontFamily: FONT, fontSize: 14, outline: 'none', boxSizing: 'border-box' }}
              />
            </div>
          ))}
          ...
          <div>
            ...
            <select value={category} onChange={e => setCategory(e.target.value)}
              style={{ width: '100%', height: 44, padding: '0 14px', borderRadius: 12, border: '1.5px solid #E5E7EB', fontFamily: FONT, fontSize: 14, outline: 'none', background: 'white' }}>
          ...
          <div>
            ...
            <textarea value={desc} onChange={e => setDesc(e.target.value)} rows={3} placeholder="Describe your product..."
              style={{ width: '100%', padding: '10px 14px', borderRadius: 12, border: '1.5px solid #E5E7EB', fontFamily: FONT, fontSize: 14, outline: 'none', resize: 'none' }}
            />
          </div>
...
function ProductCard({ p, onShowGallery }: { p: any, onShowGallery: (imgs: string[]) => void }) {
  ...
  const images = (p.image_urls && p.image_urls.length > 0) ? p.image_urls : ['https://images.unsplash.com/photo-1505740420928-5e560c06d30e?w=400&q=70'];
  ...
  return (
    ...
    >
      <div
        ...
        onClick={(e) => { e.stopPropagation(); onShowGallery(images); }}
      >
        {/* eslint-disable-next-line @next/next/no-img-element */}
        <img src={mainImage} alt={p.title} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
        
        {images.length > 1 && (
          <div style={{ position: 'absolute', bottom: 10, right: 10, background: 'rgba(0,0,0,0.6)', color: 'white', padding: '4px 8px', borderRadius: 8, fontSize: 10, fontWeight: 700, backdropFilter: 'blur(4px)' }}>
            1 / {images.length}
          </div>
        )}
        ...
        {loading && products.length === 0 ? (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(200px,1fr))', gap: 16 }}>
            {[1,2,3,4,5,6,7,8].map(n => (
              <div key={n} style={{ height: 320, borderRadius: 16, background: '#E5E7EB', animation: 'pulse 1.5s infinite ease-in-out' }} />
            ))}
          </div>
        ) : products.length === 0 ? (
          ...
          <>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(200px,1fr))', gap: 16 }}>
              {products.map(p => <ProductCard key={p.id} p={p} onShowGallery={setGalleryImages} />)}
            </div>
          </>
        )}
      </div>
